...
void initParams() {
    if ("$PLATFORM_VER" == "latest") {
        withCredentials([string(credentialsId: 'DO_TOKEN', variable: 'DIGITALOCEAN_ACCESS_TOKEN')]) {
            PLATFORM_VER = sh(script: "doctl kubernetes options versions | awk 'NF' | sed -n '2p' | awk '{print \$1}'", returnStdout: true).trim()
        }
    }
...
void createCluster(String CLUSTER_SUFFIX) {
    ...
    withCredentials([string(credentialsId: 'DO_PROJECT_ID', variable: 'PROJECT'), string(credentialsId: 'DO_TOKEN', variable: 'DIGITALOCEAN_ACCESS_TOKEN')]) {
        sh """
            export KUBECONFIG=/tmp/$CLUSTER_NAME-$CLUSTER_SUFFIX

            maxRetries=15
            exitCode=1
            cluster_version=\$(doctl kubernetes options versions | grep "^.*$PLATFORM_VER" | head -n 1 | awk '{print \$1}')
...
pipeline {
    ...
    parameters {
        ...
        string(name: 'PLATFORM_VER', defaultValue: 'latest', description: 'Digital Ocean kubernetes version')
