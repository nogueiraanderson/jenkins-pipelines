...
| **Bootstrap + finalize** (`scripts/bootstrap-ol10.sh`, `bootstrap/finalize-ol10.pkr.hcl`) | OL10 lineage root, one-time per arch | Import Oracle's official OL10 cloud image (arm64 `*-kvm-cloud-*.qcow2`, x86_64 `*-aws-*.vmdk`) via `aws ec2 import-snapshot`, register the raw base, then `packer build` the `amazon-ebssurrogate` finalize: it adds `ec2-user` + `amazon-ssm-agent` + `dnf update` AND shrinks the root onto a `var.volume_size` surrogate in one build, so the candidate is already the size the refresh launches (EBS cannot restore below a snapshot, XFS cannot shrink in place). Needed because Oracle ships no OL10 AWS AMI. `bootstrap-ol10-verify` then boot-tests two sizes + smoke + size-gate + promote. Full internals (`dd` /boot, LVM-to-plain, the gates): [docs/finalize.md](docs/finalize.md). |
...
just bootstrap-ol10 x86_64              # one-time OL10 lineage root for an arch (import + finalize-ebssurrogate to var.volume_size)
just bootstrap-ol10-verify <ami> x86_64 # two-size boot + smoke + size-gate, then promote to the consumed role
```
...
includes `10`, so OL10 refreshes on the same schedule as OL8/OL9. The finalize
build (`amazon-ebssurrogate`) emits the base at `var.volume_size` directly, so it
launches on the same root size as OL8/OL9 instead of Oracle's larger imported root
([docs/finalize.md](docs/finalize.md)); the refresh sustains that size. The one-time
