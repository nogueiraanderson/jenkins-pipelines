#!/usr/bin/env bash
# PG-2353 fail-closed identity/health gates for an Oracle Linux package-test
# target AMI. Runs as the LAST provisioner: any non-zero exit aborts the build
# so a broken or mislabeled image is never registered. Its final block
# de-instances the image (host keys + authorized_keys) once all checks pass.
#
