...
TS="$(date -u +%Y%m%d-%H%M%S)"; log_step(){ echo -e "\n=== $* ==="; }
...
if [ -n "$CAND" ] && [ "$FORCE" != 1 ]; then
  log_step "reuse existing finalized candidate $CAND (skip download/import/finalize)"
  BASE="$CAND"
else
  ...
  if [ -n "$RAW" ] && [ "$FORCE" != 1 ]; then
    ...
    if aws s3 ls "s3://$BUCKET/$VMDK" --region "$REGION" >/dev/null 2>&1; then
      ...
      log_step "download $SRC"
      [ -f "$WORK/$VMDK" ] || [ -f "$WORK/$SRC" ] || curl -fsSL -o "$WORK/$SRC" "$SRC_URL"
      log_step "convert -> $VMDK (streamOptimized; RAW's full virtual size would be a huge upload)"
      [ -f "$WORK/$VMDK" ] || qemu-img convert -O vmdk -o subformat=streamOptimized "$WORK/$SRC" "$WORK/$VMDK"
      log_step "upload to s3://$BUCKET/$VMDK"
    ...
    log_step "import-snapshot (reuse a prior completed import for this source if present)"
    ...
    log_step "register RAW base (--architecture $EC2ARCH --boot-mode $BOOTMODE)"
