# PG-2353 Oracle Linux AMI factory (refresh path).
#
...
#
# OL10 is NOT a refresh target (no self-owned OL10 base exists yet); see
# README.md for the OL10 bootstrap (import / EC2-native) spike.

packer {
  required_plugins {
    amazon = {
      source = "github.com/hashicorp/amazon"
      # >= 1.8.1: 1.7.0 shipped a session_manager panic (plugin issue #628),
      # fixed in 1.8.0; 1.8.1 fixes instance-profile propagation races.
      version = ">= 1.8.1"
    }
  }
}
...
variable "os_major" {
  ...
  default = "9"
  validation {
    condition     = contains(["8", "9"], var.os_major)
    error_message = "Value of os_major must be 8 or 9. OL10 uses the bootstrap path, not refresh."
  }
}
